...
function compileCloudManifests(config = {}) {
  const appName = (config.projectName || 'dockeryze-app').toLowerCase().replace(/[^a-z0-9\-]/g, '-');
  ...
  const k8sDeployment = `# ── Dockeryze Generated Kubernetes Deployment Manifest ─────────────────
...
`;
  ...
  const k8sService = `# ── Dockeryze Generated Kubernetes Service Manifest ────────────────────
apiVersion: v1
kind: Service
metadata:
...
`;
  ...
  const flyToml = `# ── Dockeryze Generated Fly.io fly.toml ─────────────────────────────────
...
`;
  ...
  const renderYaml = `# ── Dockeryze Generated Render.yaml Infrastructure Blueprint ─────────────
services:
  - type: web
    name: ${appName}
    env: docker
